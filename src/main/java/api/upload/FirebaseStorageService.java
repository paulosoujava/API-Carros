	...
	private void init() throws IOException {
		if (FirebaseApp.getApps().isEmpty()) {
			...
			FirebaseOptions options = new FirebaseOptions.Builder().setCredentials(GoogleCredentials.fromStream(in))
					.setDatabaseUrl("https://upload-e0bc5.firebaseio.com")
					.setStorageBucket("upload-e0bc5.appspot.com").build();
...
//        Blob blob = bucket.create("nome.txt","Paulo Ninja 2".getBytes(), "text/html");
...
/**
 * service firebase.storage { match /b/{bucket}/o { match /{allPaths=**} { allow
